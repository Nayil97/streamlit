...
class StHtmlAPITest(DeltaGeneratorTestCase):
    ...
    def test_unsafe_allow_javascript_style_only_sets_flag(self):
        """Style-only HTML still carries the JS flag when provided."""
        css = "<style>body{background:red}</style>"
        st.html(css, unsafe_allow_javascript=True)
        # First message is the event container routing; then the element
        _ = self.get_message_from_queue()
        style_el = self.get_delta_from_queue().new_element
        assert style_el.html.body == css
        assert style_el.html.unsafe_allow_javascript is True
